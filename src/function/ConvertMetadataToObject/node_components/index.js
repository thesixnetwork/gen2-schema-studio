...
class MetaFunction {
  constructor({ type, functionName, attributeName, value1, value2, negative }) {
...
class ParamFunction {
  constructor({ type, functionName, attributeName }) {
...
class Constant {
  constructor({ type, dataType, value }) {
    if (type !== "constant") {
      throw new Error("invalid type");
    }
...
class Variable {
  constructor({ type, name, negative }) {
    if (type !== "variable") {
      throw new Error("invalid type");
    }
    this.name = name;
...
class UtilityFunction {
  constructor({ type, functionName, value1, value2 }) {
