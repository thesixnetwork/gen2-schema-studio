...
export default function Page({
  ...
  const validateSchemaCode = useCallback(async () => {
    ...
    if (
      dotCountTest(schemaCode) ||
      uppercaseTest(schemaCode) ||
      spaceTest(schemaCode) ||
      specialCharsTest(schemaCode) ||
      (!findSchemaCodeStatus && schemaCode !== "")
    ) {
      setValidate(false);
    } else {
      setValidate(true);
  ...
  const create_SchemaCode = async () => {
    ...
    const createSchemaCodeStatus = await createSchemaCode(
      schemaCode,
      collectionName,
      description
    );
  ...
  const edit_schemaCode = async () => {
    const editSchemaCodeStatus = await editSchemaCode(
      schemacode,
      schemaCode,
      collectionName,
      description
    ...
    if (schemacode === "newintegration") {
      if (schemaCode !== "" && validate) {
        if (schemaCode.includes(".")) {
          ...
          setValidate(false);
          setErrorMessage(
            "Schema code should include one dot example six.whalgate "
          );
        }
      } else {
        validateNextPage();
    ...
    <>
      ...
      <div className=" w-full h-full min-h-[75vh] flex flex-col justify-between items-center  ">
        ...
        <InputCardOneLine
          ...
          value={schemaCode}
          onChange={handleInputChangeSchemaCode}
          loading={isLoadingFindSchemaCode}
        ></InputCardOneLine>
        <InputCardOneLine
        ...
        <ConfirmModalChakra
          ...
          confirmButtonTitle={"Yes, Create"}
          function={create_SchemaCode}
          isOpen={isOpen}
          setIsOpen={setIsOpen}
        ></ConfirmModalChakra>
        <ConfirmModalChakra
