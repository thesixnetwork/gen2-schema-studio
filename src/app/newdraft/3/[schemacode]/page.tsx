...
import {
  spaceTest,
  specialCharsTest,
  specialCharsTestTraitType,
  uppercaseTest,
} from "@/validateService/validate";
...
export default function Page({
    ...
    (async () => {
      try {
        ...
        if (schemaInfo) {
          ...
          const foundChain = chainMapper.find((item) =>
            item.Chain.some(
              (chain) =>
                chain.chain === schemaInfo.schema_info.origin_data.origin_chain
            )
          );

          const chainIdToSet = foundChain?.Chain.find(
            (c) => c.chain === schemaInfo.schema_info.origin_data.origin_chain
          )?.chain_id;

          // Provide a default value (e.g., an empty string) if chainIdToSet is undefined
  ...
  const getAttribute = async () => {
    if (contractAddres !== "" && contractAddres !== null) {
      try {
        const originAttribute = await getOriginAttributFromContract(
          contractAddres,
          chainId
        );
  ...
  const isAttributeExists = (newAttributeName: string) => {
    if (isDaft !== null) {
      return isDaft.schema_info.origin_data.origin_attributes.some(
        (attribute: IOriginAttributes) => attribute.name === newAttributeName
      );
    }
    return false;
  };
  ...
  const save_state3 = async () => {
    ...
    const saveState3_status = await saveState3(
      isDaft!.schema_info.origin_data.origin_attributes,
      schemacode
    );
      ...
      <div className=" w-full h-[75vh]    flex flex-col justify-between items-center ">
        ...
        {isMain ? (
          <div className="  w-full h-full  grid grid-cols-4 gap-4 overflow-scroll py-3 px-10 my-3 ">
            ...
            {isDaft !== null &&
              isDaft.schema_info.origin_data.origin_attributes.map(
                (item, index) => (
                  <div key={index}>
          ...
          <div className=" w-full h-full  flex flex-col  items-center">
            <div className=" w-full h-full min-h-[40vh] flex flex-col justify-between items-center p-10">
              ...
              <InputCardOneLine
                ...
                validate={validateStateTraitType}
                errorMassage={errorMessageTraitType}
                value={traitType}
        ...
        <ConfirmModalChakra
          title={"Are you sure go back ? "}
          confirmButtonTitle={"Yes, Go back"}
